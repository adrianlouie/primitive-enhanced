...
func NewRandomTriangle(worker *Worker) *Triangle {
	rnd := worker.Rnd
	x1, y1 := worker.RandomPosition()
	maxS := worker.ShapeMaxSize(15)
	x2 := x1 + rnd.Intn(maxS*2+1) - maxS
	y2 := y1 + rnd.Intn(maxS*2+1) - maxS
	x3 := x1 + rnd.Intn(maxS*2+1) - maxS
	y3 := y1 + rnd.Intn(maxS*2+1) - maxS
