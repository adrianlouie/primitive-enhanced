...
// solveColorAlpha analytically solves for optimal RGB color AND alpha.
// Step 1: optimal color at given alpha (same as computeColor)
// Step 2: optimal alpha given that color via closed-form MSE minimizer
func solveColorAlpha(target, current *image.RGBA, lines []Scanline, initialAlpha int) Color {
	color := computeColor(target, current, lines, initialAlpha)
	if color.A == 0 {
		return color
	}
	...
	for _, line := range lines {
		i := target.PixOffset(line.X1, line.Y)
		for x := line.X1; x <= line.X2; x++ {
			tr := float64(target.Pix[i])
			tg := float64(target.Pix[i+1])
			tb := float64(target.Pix[i+2])
			cr := float64(current.Pix[i])
			cg := float64(current.Pix[i+1])
			cb := float64(current.Pix[i+2])
			i += 4

			dr := float64(color.R) - cr
			dg := float64(color.G) - cg
			db := float64(color.B) - cb

			num += (tr-cr)*dr + (tg-cg)*dg + (tb-cb)*db
	...
	if den > 1e-6 {
		...
		color.A = clampInt(int(a*255), 1, 255)
	}
	return color
}
