...
func NewRandomQuadratic(worker *Worker) *Quadratic {
	rnd := worker.Rnd
	px, py := worker.RandomPosition()
	x1 := float64(px)
	y1 := float64(py)
	maxS := float64(worker.ShapeMaxSize(20))
	x2 := x1 + rnd.Float64()*maxS*2 - maxS
	y2 := y1 + rnd.Float64()*maxS*2 - maxS
	x3 := x2 + rnd.Float64()*maxS*2 - maxS
	y3 := y2 + rnd.Float64()*maxS*2 - maxS
	width := 1.0 / 2
